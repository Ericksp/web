...
$sql = "SELECT * from misa order by desmisa asc ";
...
<head>
    ...
    <script src="../js/siguiente.js"></script>
        ...
        <div class="text">
            <section class="form-register" id="servicioForm">
                ...
                <p>Llene el formulario para poder adquirir el servicio:</p>
                    ...
                    <p>Datos de la persona que se ofrecerá la misa :</p>
                    <label for="nombre">Nombre(s):</label><br>
                    <input class="txtbox" type="text" id="nombre2" name="nombre2" required placeholder="Esriba su(s) nombres(s)" ><br>
                    <label for="nombre">Apellidos</label><br>
                    <input class="txtbox" type="text" id="apellidos2" name="apellido2" required placeholder="Esriba su(s) apellido(s)"><br><br>
            ...
            <section class="form-register" id="segundoPaso" style="display: none;">
                    ...
                    <h2>Registre sus datos : </h2>
                <p>Llene el formulario para poder adquirir el servicio:</p>
                <label for="nombre2">¿Que Tipo de misa desea ? :</label><br><br>
                    <label  for="opciones">Selecciona una opción:</label>
                    <select class="txtbox" id="opciones" name="opciones">
                    <option value="">Seleccione</option>
                    <?php
                            while ($row = $result->fetch_assoc()) { ?>
                        <option value="<?php echo $row['codmisa']; ?>" data-precio="<?php echo $row['premisa']; ?>">  <?php echo $row['desmisa'] .' -  S./'.  $row['premisa'] ; ?></option>

                        <?php } ?>
                        </select>

                     
                    <p>Seleccione la fecha de la misa :
                    <input class="txtbox" type="text"  id="datepicker" name="fecha" placeholder="Seleccione la fecha de la misa">
                    ...
                    <button type="submit" class="btn-1" id="btnPagar" style="display: none;">Pagar</button>
            </section>
            ...
            <input type="hidden" id="precio" name="precio" value="">
                ...
                <input type="hidden"  name="hora_format" value="">
...
$(document).ready(function () {
        $("#opciones").change(function () {
            ...
            $("#datepicker").val('');
            $("#btnPagar").hide();
            $("#hora").empty().append('<option value="0">No hay horarios disponibles</option>').val('0');
        });
        ...
        if (opciones === '') {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Por favor, seleccione una opción en el campo "Opciones".',
            });
    ...
    flatpickr("#datepicker", {
        ...
        onChange: function(selectedDates, dateStr, instance) {
            var fechaSeleccionada = dateStr;
            var idComboSeleccionado = $("#opciones").val(); // Captura el ID del combo opciones
            $.ajax({
                url: 'select-horas.php',
                type: 'POST',
                data: { fecha: fechaSeleccionada, idCombo: idComboSeleccionado }, // Envía ambos valores por AJAX
                success: function(response) {
                    console.log(response)
                    $("#horasDisponibles").html(response);
                   

                    var selectValue = $("#hora").val();
                    var selectText = $("#hora option:selected").text(); 


                    $("input[name='hora_format']").val(selectText);
                   
                    if (selectValue === '0') {
                        $("#btnPagar").hide(); // Ocultar el botón
                    } else {
                        $("#btnPagar").show(); // Mostrar el botón
                    }
